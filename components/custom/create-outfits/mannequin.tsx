...
function ClothingLayer({ type, className = '' }: ClothingLayerProps) {
  return (
    <div className={`absolute ${className}`}>
      {type === 'shirt' && (
        <svg width="100" height="120" viewBox="0 0 100 120" className="text-gray-200">
          <path d="M20 20 H80 V90 C80 100 60 110 50 110 C40 110 20 100 20 90 V20" 
                fill="currentColor" />
        </svg>
      )}
      {type === 'pants' && (
        <svg width="100" height="150" viewBox="0 0 100 150" className="text-gray-200">
          <path d="M30 0 H70 L75 140 H55 L50 150 L45 140 H25 L30 0" 
                fill="currentColor" />
        </svg>
      )}
    </div>
  ...
  return (
    <div className="relative h-full w-full flex items-center justify-center">
      <div className="relative h-[700px] w-64">
        ...
        <div className="absolute top-[320px] left-1/2 -translate-x-1/2 w-32 flex justify-between"> {/* Reduced width from 36 to 32 */}
          {/* Left Leg */}
          <div className="relative w-8"> {/* Reduced from w-10 */}
            <div className="w-8 h-60 bg-neutral-300 rounded-t-xl transform translate-x-1 rotate-[1deg]"> {/* Added translate-x-1 */}
              {/* Calf */}
