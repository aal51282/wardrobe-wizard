...
export default function ProductPage() {
  ...
  const [items, setItems] = useState([
    { id: 1, name: 'Skinny Mid-Rise Trousers', image: '/images/trousers.jpg', category: 'Men', selected: false },
    { id: 2, name: 'Striped Shirt', image: '/images/shirt.jpg', category: 'Women', selected: false },
    { id: 3, name: 'Sunglasses', image: '/images/sunglasses.jpg', category: 'Accessories', selected: false },
  ]);
      ...
      <div className={styles.itemsGrid}>
        {filteredItems.map((item) => (
          <div key={item.id} className={styles.itemCard}>
            <Image src={item.image} alt={item.name} width={150} height={150} className={styles.itemImage} />
            <h3 className={styles.itemName}>{item.name}</h3>
            <div className={styles.itemButtonsContainer}>
              <button
                className={item.selected ? styles.deselectButton : styles.addButton}
